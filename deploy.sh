...
docker-compose exec -ti ovpn_instance sh -c "cp /etc/openvpn/MyReq.key /opt/crt/server.key"
docker-compose exec -ti ovpn_instance sh -c "cp /etc/openvpn/MyReq.crt /opt/crt/server.crt"
